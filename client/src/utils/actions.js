...
// This is great for performance, but it also means we have to go through Apollo every time we want 
// to update that list. The end goal here is to store the data retrieved for products by Apollo in 
// this global state. This way, we can add offline capabilities later and persist our product data!
...
// in that we want to be able to select a category from the state created by the UPDATE_CATEGORIES action and 
